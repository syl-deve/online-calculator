exports.handler = async function(event, context) {
    try {
        const { robuxInput, krwInputRobux, usdInput, platform, source } = JSON.parse(event.body);
        ...
        let robux_USD_TO_KRW = 1380; // Default value

        // Fetch latest USD to KRW rate
        try {
            const response = await fetch('https://open.er-api.com/v6/latest/USD');
            if (response.ok) {
                const data = await response.json();
                robux_USD_TO_KRW = data.rates.KRW;
            } else {
                console.warn('Failed to fetch latest USD to KRW rate, using default.');
            }
        } catch (error) {
            console.error('Error fetching USD to KRW rate:', error);
            console.warn('Using default USD to KRW rate.');
        }
