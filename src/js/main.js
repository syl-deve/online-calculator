document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupRobuxCalculator() {
        ...
        let robux_USD_TO_KRW_rate = 1380; // Default value, will be updated by fetchRobuxRate
        ...
        async function fetchRobuxRate() {
            try {
                ...
                if (!response.ok) throw new Error('Network response was not ok');
                const data = await response.json();
                robux_USD_TO_KRW_rate = data.rates.KRW;
                robuxRateInfo.textContent = `현재 환율 적용 중: 1달러 = ${robux_USD_TO_KRW_rate.toFixed(2)}원`;
            } catch (error) {
                console.error("환율 정보 가져오기 실패:", error);
                robuxInfoBox.className = 'bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-lg mb-6';
                robuxRateInfo.textContent = `실시간 환율 로딩 실패. 예상 환율(1달러 = ${robux_USD_TO_KRW_rate}원)을 기준으로 계산합니다.`;
            }
        }
            ...
            try {
                const response = await fetch('/.netlify/functions/calculate-robux', {
                    ...
                    body: JSON.stringify({
                        ...
                        source: source,
                        robux_USD_TO_KRW: robux_USD_TO_KRW_rate // Pass the fetched rate to the backend
                    }),
                });
                ...
                // Update info text with the rate used by the backend
        ...
        // Initial setup
        updateRobuxInfoText();
        fetchRobuxRate(); // Fetch rate on initialization
    }
